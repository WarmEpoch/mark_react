          ...
          imgs.map(img =>
            <Draw key={img.id} img={img} border={4}>
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexFlow: 'column' }}>
                ...
                <p style={{ fontWeight: 'bold', fontSize: img.width > img.height ? '.86em' : '.82em', lineHeight: img.width > img.height ? '3.6em' : '2.8em' }}>{img.reveals.h1}{img.reveals.h2 && ` | ${img.reveals.h2}`}</p>
              </div>
            </Draw>
          )
        }
